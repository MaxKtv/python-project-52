...
name = models.CharField(
    ...
    verbose_name=_('Name'),
    error_messages={
        'unique': _("Object with this name already exists."),
    }
)
...
def delete(self, *args, **kwargs):
    ...
    if hasattr(self, 'task_set') and self.task_set.exists():
        raise models.ProtectedError(
            "Cannot delete object because it has related tasks",
            self.task_set.all()
        )
...
def get_success_url(self):
    ...
    if not hasattr(self, '_success_url'):
        app_label = self.model._meta.app_label
        # Для модели User из auth приложения используем users namespace
        if app_label == 'auth' and self.model._meta.model_name == 'user':
