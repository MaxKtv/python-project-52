...
class BaseCRUDTest(TestCase):
    ...
    def get_url(self, action, args=None):
        ...
        url_name = (
            self.base_url_name if action == 'list'
    ...
    def assert_view_response(self, response, template_suffix):
        ...
        self.assertTemplateUsed(
            response,
            self.get_template_path(template_suffix)
        )
    ...
    def test_delete_protected(self):
        ...
        task_model.objects.create(
            ...
            status=self.instance if isinstance(
                self.instance,
                task_model._meta.get_field('status').related_model
            ) else None,
            executor=self.user,
            author=self.user,
            label=self.instance if isinstance(
                self.instance,
                task_model._meta.get_field('label').related_model
            ) else None,
        )
    ...
    def test_unauthorized_access(self):
        ...
        for action, args in urls:
            ...
            self.assertRedirects(
                response,
                f'/login/?next={self.get_url(action, args)}'
            )
